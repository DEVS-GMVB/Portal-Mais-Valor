...
let banco = document.getElementById('banco-filtro')
let bancoInsert = document.getElementById('banco-campo')
...
window.onload = function(){
    ...
    .then(response => response.json().then(function(data){
        for(let i = 0; i < data.length; i++){
            banco.innerHTML += '<option value="' + data[i].banco + '">' + data[i].banco + '</options>'
            bancoInsert.innerHTML += '<option value="' + data[i].banco + '">' + data[i].banco + '</options>'
        }
